...
#
# This is a simple filter that compresses PDF-files using pdftk, which needs
# to be in your PATH. Together with filter_pdf_clean.rb, this can increase the
# storage efficiency inside a git-repository.
#
# NOTE: Input and output will _not_ be identical, since pdftk's compress and
#       uncompress functions are not directly inverse. If you need to have PDF
#       files that have to pass an md5sum check or something, *don't* use this
#       script.
#
...
begin
  ...
  # Try compressing it using pdftk
  Open3.popen3("pdftk - output - compress") do |stdin, stdout, stderr|
    stdin.print(input)
    stdin.close
    output = stdout.read
    errors = stderr.read
